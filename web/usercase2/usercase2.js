...
function renderOffline(data) {
  ...
  if (list.length === 0) {
    tbody.innerHTML = `
      <tr>
        <td class="px-4 py-4 text-slate-500" colspan="3">Không có thiết bị offline theo ngưỡng hiện tại.</td>
      </tr>
    `;
    return;
  }
...
async function runChecks() {
  ...
  const minutes = Number($("offline-minutes")?.value || 5) || 5;
  const limit = Number($("alerts-limit")?.value || 50) || 50;

  const offlineUrl = `${base}/api/daocuong/devices/offline/check?minutes=${encodeURIComponent(minutes)}`;
  const thresholdUrl = `${base}/api/daocuong/threshold/check`;
  const alertsUrl = `${base}/api/daocuong/alerts?limit=${encodeURIComponent(limit)}`;
